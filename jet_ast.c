...
{    
    ...
    jet_list* top_node_list = jet_list_create(16, sizeof(jet_ast_node));
    if(!top_node_list)
    {
...
bool jet_ast_dispose(jet_ast* ast)
{
    ...
    {
        jet_ast_node* node = jet_list_get(ast->top_node_list, i);
        if(node) 
...
static jet_token* jet_ast_consume_tok(jet_ast* ast)
{
    ...
    if(ast->tok_cursor == jet_list_count(ast->tok_list) - 1)
    {
        puts("end of token-list reached.");
        return NULL;
    }
